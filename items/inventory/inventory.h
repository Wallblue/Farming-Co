...
//Functions :
    ...
    unsigned char setItem(const Item* item, Inventory inventory); //Set a new item in the inventory and return 0 if it worked 1 else
    unsigned char addItem(const Item* item, Inventory inventory); //Add parameter item to existing items or set it if no existing ones
    short firstEmptySlot(const Inventory inventory); //Return the first slot's index without item, -1 if not anymore

    unsigned char substractItem(int id, unsigned char quantity, Inventory inventory); //Substract a given quantity of an item
